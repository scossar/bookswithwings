...
<form action="<?php echo $action; ?>" method="post" id="submit-donation-form"
	...
	<?php if ( wpdonations_user_can_post_donation() ) : ?>
			...
			<fieldset class="fieldset-<?php esc_attr_e( $key ); ?>">
				<div class="form-group">
				  <label for="<?php esc_attr_e( $key ); ?>"><?php echo $field['label'] . ( $field['required'] ? '' : ' <small>' . __( '(optional)', 'wpdonations' ) . '</small>' ); ?></label>
					<div class="field">
						<?php get_wpdonations_template( 'templates/form-fields/' . $field['type'] . '-field.php', array( 'key' => $key, 'field' => $field ) ); ?>
					</div>
				</div>
			</fieldset>
		<?php endforeach; ?>
		...
		<p>
			...
			<input type="submit" name="submit_donation" class="button btn btn-primary" value="<?php esc_attr_e( $submit_button_text ); ?>" />
		</p>
